...
func (n *klusterletController) cleanUp(
	...
	// The klusterlet namespace on the management cluster should be removed at last. Otherwise if any failure occurred,
	// the managed-external-kubeconfig would be removed and the next reconcile will fail due to can not build the
	// managed cluster clusters.
	if config.InstallMode == operatorapiv1.InstallModeDetached {
